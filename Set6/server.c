...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char * parts[4];
            int i = 0;
            //Split the request line on whitespace. If by mistake there are whitespaces
            //in any of the parts it will just have too many parameters and return 400.
            //If by mistake there are more whitespaces between parts instead of one the function ignores them. 
            ...
            //If there are more parameters than 3 throw error (Bad request)
            if (i != 3){
                error(400);
                continue;
            }
            //Close the array
            parts[3] = "\0";
            //If method is not GET throw error (Method not allowed)
            if (strcmp(parts[0], "GET") != 0){
                error(405);
                continue;
            }
            //If the request-target does not start with '/' or does not contain extension delimiter '.' throw error (Not implemented)
            if (parts[1][0] != '/' || strchr(parts[1], '.') == NULL){ 
                error(501);
                continue;
            }
            //If the request-target contains the symbol " throw error (Bad request)
            if (strchr(parts[1], '\"') != NULL){
                error(400);
                continue;
            }
            //If HTTP version is not 1.1 throw error (HTTP version not supported)
            if (strcmp(parts[2], "HTTP/1.1\r\n") != 0){
                error(505);
                continue;
            }
            // Extract query from request-target
            //Make pointer point to first occurence of the symbol "?" in the request-target (That represents the start of query)
            char * q_holder = strchr(parts[1], '?');
            //At start assume that size is going to be 1 and will be changed if the query exists
            int q_size = 1;
            //If the query exists change the size variable and skip the character "?" in the query
            if (q_holder != NULL && (q_holder + 1) != NULL){
                q_size = strlen(q_holder); 
                q_holder++;
            }
            //When the proper size is determined initialise a string with that size
            char query[q_size];
            //If the size is bigger than 1 then copy the query 
            if (q_size > 1){
                strcpy(query, q_holder);
            }
            //Close the string
            query[q_size - 1] = '\0';
            //Just to see what the query looks like
            printf("Query: %s\n", query);
            
            
            // Concatenate root and absolute-path
            //Determine the size of the concatenated string
            int path_length = strlen(root) + strlen(parts[1]) + 1;
            //Initialise the string with the determined size
            char path[path_length];
            //Copy the root part to the new string (We need this so it doesn't concatenate the original root.
            strcpy(path, root);
            //Concatenate root and abolute path
            strcat(path, parts[1]);
            //Close the string
            path[path_length - 1] = '\0';
            //Just to see what the path looks like
            printf("Path: %s\n", path);

            // Ensure path exists, if not throw error (File not found)
            if (access(path, F_OK) != 0){
                error(404);
                continue; 
            }
            
            // Ensure path is readable if not throw error (Forbidden)
            if (access(path, R_OK) != 0){
                error(403);
                continue;
            }
            // Extract path's extension
            //Point to the last occurence of symbol "." in the path string
            char * e_holder = strrchr(path, '.');
            //Assume the size of extension to be 1 initially
            int ext_length = 1;
            //If extension exists update size of the extension and skip the "." character
            if (e_holder != NULL && (e_holder + 1) != NULL){
                ext_length = strlen(e_holder);
                e_holder++;
            }
            //Initialise extension holder string to the determined size
            char extension[ext_length];
            //If extension length is bigger than 1 copy the extension from the holder
            if (ext_length > 1){
                strcpy(extension, e_holder);
            }
            //Close the extension string
            extension[ext_length - 1] = '\0';
            //Just to see what the extension looks like
            printf("Extension: %s\n", extension);
            ...
            else
            {
                ...
                // Respond to client while checking for errors in each print line
                if (dprintf(cfd, "HTTP/1.1 200 OK\r\n") < 0){
                    continue;
                }
                if (dprintf(cfd, "Connection: close\r\n") < 0){
